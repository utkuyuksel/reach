...
- **v1.7 — LiveOps temeli (F2P denetiminin uygulanması).** `_spec/design-audit-2026-06.md`'deki Faz 1+2 ve Faz 3'ün büyük kısmı tek sürümde:
  - **Ekonomi onarımı:** ödüllü reklam 25→15 (arbitraj kapandı; ipucu-funnel'da reklam artık coin yerine DOĞRUDAN ipucu verir), kazanım ekranında **×2 coin** ödüllü butonu, ana ekranda **günlük hediye reklamı** (+40), **ilk interstitial ≥12 clear** (Vita Mahjong kuralı). Paket eğrisi onarıldı: 200/800/2200 ($0.99/2.99/6.99 — artık monoton: 202→268→315 c/$). **Premium ayrıştırıldı:** $7.99, sınırsız yerine **günde 5 bedava ipucu** + reklamsız + tüm temalar. **Starter Pack** $1.99: 500 coin + **Ember** (exclusive, coinle alınamaz, Premium'a dahil değil) + 1 Freeze.
  - **Streak ekonomisi:** **Streak Freeze** (150c, max 2, kaçan günü otomatik emer) + **Streak Repair** (60c, 48 saat pencere, ayda 1, takvimde sessiz kart — asla popup) + **bayat-gösterim düzeltmesi** (`effectiveStreak`) + **saat-koruması** (20s debounce + yalnız bugünün tahtası; takvim kaydı her zaman, streak kredisi korumalı).
  - **Takvim + madalyalar (D30 omurgası):** ay ızgarası (canlı/backfill/3★ ayrımı), **bronz 20 / gümüş 25 / altın tüm ay**, kupa rafı, ay gezintisi. **Arşiv/backfill:** kaçan gün 40c (medal sayılır, streak asla), ücretsiz son 7 gün / Premium tamamı, arşiv kazancı 21c. `GameMode.archive` eklendi.
  - **Zen yeniden yapılandırıldı:** `clearsPerLevel=10` → rozet ve %10 bar nihayet aynı şeyi söylüyor (**bölümler**); bölüm içi **testere-dişi** decoy modülasyonu (0-6 yükseliş, 7-8 nefes, 9 **finale** — elmas ikonu, çift coin, bölüm sandığı +40); **görünmez merhamet direktörü** (3+ yanlış iz veya stuck → sonraki 2 tahta bandın tabanında); **flow chain** (temiz seri; 3/7/15'te +10/20/40, ZenRecord'da chain/bestChain).
  - **Meta:** **rozet rafı** (8 kelimesiz rozet × 3 kademe; popup yok), `StatsRecord` (toplam/temiz clear, 3★ daily), ana ekran reworku (coin chip + hediye + rozet + ayarlar şeridi; Daily bitince **"sonraki bulmaca HH:MM"** geri sayımı; karta dokunma → takvim; Premium yoksa sessiz shop satırı).
  - **Kozmetik katalog 4→13:** Tide/Honey 600, Rosewood/Moss 800, Sand/Plum 1000, Slate/**Midnight (ilk koyu tema)** 1200 + Ember (exclusive). Katalog değeri 800→~9.000c.
  - **Daily Ladder:** aynı tarihten **4×4 (+8c)** ve **6×6 (+24c)** kardeş tahtalar (deterministik seed×3+tier; `GameMode.ladder`, anahtar `YYYY-MM-DD#e/#h`). Streak'e ve madalyaya ASLA dokunmaz — Medium tek kanonik paylaşım tahtası kalır; NYT "çoklu günlük alışkanlık noktası" mekaniği. Giriş: takvimde bugün-satırı.
  - 4 yeni test dosyası/grubu: streak (freeze/repair/saat/madalya), ekonomi (starter/exclusive/kota/hediye/eğri), controller (finale/bölüm/chain/arşiv/ladder). Tüm suite yeşil.
  - **Adversarial review (3 ajan) + düzeltmeler:** (1) **coin pompası tıkandı** — kazan→restart→tekrar kazan artık Daily/arşiv/ladder ödülünü YENİDEN basmıyor (ödül yalnız taze kayda; pre-existing kaçak); (2) **backfill bileti** — 40c giriş ücreti kalıcı bilet (`paidBackfills`), yarıda bırakmak ücreti yakmaz, tamamlanınca tüketilir; (3) **saat modeli dürüstleştirildi** — süre-debounce'u kaldırıldı (ileri-saran saldırgan görünen süreyi de kontrol eder; debounce yalnız dürüst gece-yarısı oyuncusunu cezalandırıyordu); kalan korumalar: bugün-anahtarı + geri-sarmada kredisizlik + gün idempotensisi; ileri-sarma riski bilinçli kabul (yalnız coin, düşük bahis); (4) giftDateKey bellek aynası, geri sayım gece-yarısı kelepçesi.
- **v1.15 — Modifier TANITIM LEVELLERİ + taş rakamları DM Mono (playtest 2. tur).**
  - **Tanıtım levelleri (sektör standardı):** her modifier'ın debut leveli artık elle-tasarlanmış, kolay, REHBERLİ bir tahta — LV 11 altın, LV 21 sis, LV 31 kilit, LV 61 joker. 2×3, hedef 12, yalnız iki geçerli yol (ders atlanamaz); oyun ekranı tutorial'ın kayan parmağı + halkalarıyla sıradaki grubu koçluyor. Tanıtım = levelin kendisi (ilerleme normal); leveller deterministik olduğundan herkes aynı tanıtımı aynı levelde görür. 7 test (sıralı çözüm, kilit kapısı, sis kalkışı, joker emmesi, decoy'suzluk).
  - **Taş rakamları DM Mono:** Fraunces'ın eski-stil serif rakamları (özellikle 3/4/5) taş boyutunda yapısal olarak bulanık — opsz/lining denemesi yetmedi; sayı-bulmaca standardına geçildi (Sudoku.com/Number Match): taşlar DM Mono w500, hedef sayı + başlıklar Fraunces'ta kalır. Ghost'lar uyumlu. NOT: iki kez python `\$`-kaçış hatası replace'leri sessizce düşürdü (ana ekran LV metni + taş fontu) — string-interpolasyonlu Dart satırlarında artık Edit aracı kullanılacak.
  - Ana ekran Zen kartı: "LV 36" (eski 'LV4 · 5/10' kalıntısı temizlendi).
- **v1.14 — Level-per-board + deterministik leveller + font netliği (playtest düzeltmeleri).**
  - **1 tahta = 1 level** (kullanıcı kararı: "çok fazla level hissi"): rozet/etiketler artık LV = temizlenen+1. 10'luk döngü İÇ yapı olarak kaldı: her 10. level elmas-ikonlu **kilometre taşı** (çift coin + sandık), modifier takvimi aynı döngüde.
  - **Deterministik leveller:** Zen seed'i artık `f(level)` — **Level N herkes için aynı tahta** (topluluk karşılaştırılabilirliği, ileride level-bazlı içerik/leaderboard). Tek istisna: görünmez merhamet direktörü zorlanan oyuncuya o levelin daha nazik bir varyantını verebilir (bilinçli takas). Test: iki bağımsız konteyner aynı Level 1 tahtasını üretir.
  - **Oyun ekranında level:** Zen çipi "ZEN" yerine canlı **"LV n"**.
  - **Font netliği:** taş rakamları bulanıktı — kök neden Fraunces'ın 144-opsz display kesimi + eski-stil rakamlar. Yeni `AppText.tileNumber`: **opsz 18 (text kesimi) + ağırlık 640 + lining/tabular rakamlar** → küçük boyutta kalın-net, 3/4/5 taban hizasında. Ghost'lar da aynı kesimde.
  - **Örtülü-taş öğretme dokunuşu:** "?"a tek dokunuş artık taşı + komşularını ipucu halkasıyla 0.65sn parlatıyor — "komşu temizlenince açılır"ın kelimesiz cevabı (sahibinin kendisinin sormak zorunda kalması üzerine).
- **v1.13 — JOKER (✦) + LV etiketi + koyu-tema shop düzeltmesi.**
  - **Joker (bölüm 7+):** sayı yerine ✦; ize katıldığında **eksik kalan değeri kendisi emer** (≥1). Tahta başına en fazla 1; her finalde + ara tahtaların ~yarısında. **Gizli-değer mimarisi:** taş içeride inşa değerini saklar → partition/hint/safe-rewind solver'ları DEĞİŞMEDEN kesin kalır; yalnız oyuncuya dönük temizleme kuralı esner (`isClearable`: S ≤ hedef−W). `hasMove` joker-farkında (kilitli olmayan komşusu olan joker = her zaman hamle; izole joker = gerçek çıkmaz). Toplam çipi "n+✦" gösterir; match-yeşili esnek kurala göre yanar.
  - **CH → LV:** kullanıcı kararı — oyuncuya dönük etiket "LV n · k/10" (yapı aynı: 10 tahta + finale + sandık; iç isimler chapter kaldı).
  - **Shop koyu-tema düzeltmesi:** tema kartları kendi paletinin ink/accent'ini kullanır (Midnight/Ember koyu-üstüne-koyu görünmezliği bitti; koyu tema seçiliyken açık kartların silinmesi de aynı kök nedendi). Simülatörde doğrulandı.
  - **Debug playtest kısayolu:** Zen kartına uzun bas = +10 tahta (yalnız kDebugMode; mağaza derlemesinde yok).
  - Testler: joker emme kuralı/alt-sınır, hasMove kısayolu + izole-joker çıkmazı, gizli-değerle partition/hint, 200-seed dört-modifier birlikte replay-to-win.
- **v1.12 — Gerçek müzik entegre.** Kullanıcının Pixabay'den seçtiği 5 parça (Leberch, Morgan/Dreamscape, Paul Yudin, The Mountain ×2): 256kbps MP3 → **112kbps AAC/M4A** (`afconvert`, 22MB→9.5MB), rastgele başlangıçlı sıralı rotasyon, `LICENSES.md` defteri dolduruldu (Pixabay Content License — ticari serbest, atıfsız). Parça eklemek = dosya + 1 satır; hedef zamanla 15-20 parça (o noktada bundle boyutu için on-demand indirme düşünülecek). Sentezlenen pad artık kullanılmıyor. Simülatörde asset-yükleme hatasız doğrulandı.
- **v1.11 — Mozaik sandığı + bağlamsal öğretme + müzik altyapısı (ortak karar turu).**
  - **Kararlar (ikimiz):** lansmanda **üyelik YOK** (tür standardı hesapsız; Google girişi Apple-girişini ve hesap-silme yükünü zorunlu kılar; sonra: anonim Firebase Auth → opsiyonel giriş). **Veritabanı şimdi değil** (local-first `StorageService` zaten bulut-takılabilir; gerektiğinde Firestore). **Joker kendi turunda** (toplam semantiğini değiştiren tek modifier).
  - **Mozaik sandığı:** eser tamamlanınca **+60c** (haftada 1, farm edilemez) — kazanım ekranındaki coin toplamına eklenir; galeri kalıcı kupa.
  - **Bağlamsal öğretme (büyük tutorial DEĞİL):** ilk mozaik hücreleri dolunca ana karttaki esere **iki nefeslik nabız** (bir kez, `seenMosaicIntro`); **kilitli taşa dokunma** artık reject-flaşı + hafif haptik veriyor ("neden çizemiyorum" cevabı — kalıcı geri bildirim); ayarlara **"How things work"** kelime-az ikon-kart yardım sayfası.
  - **Müzik:** sentezlenen ambient placeholder ilan edildi; `AudioPlayersSoundService` çoklu-parça rotasyona geçirildi (`_ambientTracks` listesi — parça eklemek = dosya + 1 satır), `assets/music/LICENSES.md` lisans defteri (kural: **Pixabay Music**, ticari serbest, atıfsız; ~96-128kbps M4A, <2MB/parça). Gerçek parçaları kullanıcı dinleyip seçecek.
- **v1.10 — Mozaik etkinliği + ana ekran revizyonu + Kilitli taş.**
  - **Haftalık mozaik (her zaman açık, opt-in'siz):** her TAZE tahta temizliği (tüm modlar) o haftanın 7×7 deterministik üretken "sakin kilim"inden 3 hücre açar (Premium +1). Pazartesi devrilir; biten/bitmeyen eser **kalıcı galeriye** kaydedilir (Block Blast Adventure'ın unuttuğu galeri). Sanat saf matematik: haftalık seed → 3 ton + 4 motif + 4-yönlü ayna simetrisi, sıfır asset. Ana ekranda yarı-açılmış mini mozaik kartı = kelimesiz ilerleme; dokun → galeri.
  - **Ana ekran revizyonu:** Daily kartında **son 7 gün nokta şeridi** (dolu/bugün-halka/soluk), Zen kartında dürüst **"CH n · k/10"**, araya mozaik kartı.
  - **Kilitli taş (bölüm 4+: 1, bölüm 6+: 2):** kilitli taş izlenemez; komşu bir grup temizlenince açılır → toplamların üstüne sıralama bulmacası. Üretimde DOĞRULAMALI yerleşim (her kilidin farklı-gruptan anahtar komşusu; grup başına tek kilit; zincirler çözülebilir — 300-seed sweep kanıtı). Solver farkında: `hasMove` kilitliyi sayma (tamamen kapılı tahta = gerçek stuck), `hint` asla kilitli grubu göstermez. UI: kilit rozeti, iz kilitliye uzanmaz.
  - **Joker (wildcard) BİLİNÇLİ ERTELENDİ:** toplam semantiğini değiştiriyor (isClearable/hasMove/decoy metriği) — kendi dikkatli turunu hak ediyor; aceleyle gömülecek iş değil.
  - Testler: kilit kapısı/açılışı, kapılı-tahta stuck, kilitli-grup hint filtresi, 300-seed yerleşim+sweep, mozaik açılım/clamp/Premium bonusu/hafta devri.
- **v1.9 — Modifier taşlar, faz 1: Altın + Örtülü.** Zen'in uzun-vadeli çeşitlilik motoru başladı (denetim Z4; bölüm başına TEK yenilik kuralıyla):
  - **Altın taş** (bölüm 2+): tahta temizlenince taş başına +5c; her finalde garanti 1 (bölüm 5+'ta 2), ara tahtalarda ~1/3. Köşede coin parıltısı (şekil, renk değil).
  - **Örtülü taş** (bölüm 3+, 2→6 ölçekli): değer "?" arkasında; KOMŞU bir grup temizlenince açılır → tarama stratejisi "kenarları temizle, sisi aç"a döner. Örtülü taş izlenebilir (değerler gerçek); iz örtülü taş içerirken **toplam çipi "?"** gösterir — tek ipucu match-yeşili (sisi yoklamak oyunun kendisi).
  - **Mimari ilke korundu:** `Generator.decorate` partition'dan SONRA süsler — değer/grup/çözülebilirlik dokunulmaz (testle kanıtlı); seed'e deterministik. Daily/Ladder/arşiv temiz kalır (yalnız Zen). `Tile.modifier` + `GameState._unveilAround` motor ekleri.
  - Testler: decorate sayım/determinizm/değer-dokunulmazlığı, örtü açılma kuralı, sis içinden iz, altın bonus, **2k süslenmiş tahta replay-to-win stress**.
  - Sırada (faz 2): kilitli taş (üretim-anı sıralama doğrulaması) + joker (solver semantiği) → sonra mozaik + ana ekran revizyonu.
- **v1.8 — Oyun hissi (juice) + Güvenli Ana Sar.** Gameplay-eğlence endişesine ilk cevap:
  - **Ghost-pop:** temizlenen taşlar sessizce yok olmak yerine match-yeşili bir "yankı" bırakıyor — büyür, hafif yükselir, solar; iz sırasına göre 45ms kademeli (dalga etkisi). Simülatörde animasyon ortasında görsel doğrulandı.
  - **Ses katmanlama:** clear sesi tahta içi combo ile yarım-ton tizleşiyor (`clear(combo:)`, playbackRate 1.0→1.225 cap; akışı duyulur kılar, asla cıyaklamaz).
  - **Güvenli Ana Sar (kullanıcı kararı: reklamlı):** çıkmaz overlay'inde üçüncü/birincil buton — tek dokunuş + 1 ödüllü reklam, tahtanın hâlâ çözülebilir olduğu son hamleye otomatik sarar (`rewindToSafe`, `Solver.isSolvable` ile geri yürür). Premium'a anında/bedava (sessiz perk). Undo/restart bedava kalır. Overlay modal kaldı (kullanıcı tespiti: uyarı zaten ancak son hamlede gelir, tahta o an görünürdür — banner gereksiz).
  - Onaylı sıranın kalanı: modifier taşlar → mozaik + ana ekran revizyonu → TestFlight.
